...
export default function ChartComponent() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        setChartData((prevData) => {
          ...
          if (JSON.stringify(prevData) !== JSON.stringify(newDataset)) {
            return newDataset;
          }
          return prevData;
        });
    ...
    fetchData();
    polling = setInterval(fetchData, pollInterval); // Poll every 5 seconds

    return () => clearInterval(polling); // Clean up interval
  }, []);
